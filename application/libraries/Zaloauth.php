...
class Zaloauth {
  ...
  public function __construct() {
    ...
    $app_id = ZALO_APP_ID_CFG;
    $app_secret = ZALO_APP_SECRET_KEY_CFG;
...
//      dd($this->accessTok);
  ...
  function getAccessTokenByApp() {
    try {
      ...
      if (!isset($_COOKIE[$cookie_name])) {
        ...
        $_COOKIE['access_token']=$accessToken->getValue();
      } else {
  ...
  function getMe() {
    ...
    if (!isset($_COOKIE[$cookie_name])) {
      ...
      $response = $this->zalo->get('/me', ['fields'=>'id, name, birthday, picture, gender'],$accessToken, Zalo::API_TYPE_GRAPH);
  ...
  function getFiends() {
    ...
    if (!isset($_COOKIE[$cookie_name])) {
      ...
      $response = $this->zalo->get('/me/friends', $params,$accessToken, Zalo::API_TYPE_GRAPH);
      echo '<br><br>';
  ...
  function getInvitableFiends() {
    ...
    if (!isset($_COOKIE[$cookie_name])) {
      ...
      $accessToken = $_COOKIE[$cookie_name];
      $params = ['offset' => 0, 'limit' => 10, 'fields' => "id, name"];
      $response = $this->zalo->get('/me/invitable_friends', $params,$accessToken, Zalo::API_TYPE_GRAPH);
      echo '<br><br>';
      print_r($response->getDecodedBody());
      echo '<br><br>';
    }
