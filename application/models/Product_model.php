...
class Product_model extends STEVEN_Model
{
    ...
    public function _where_custom($args = array()){
        ...
        if (!empty($search_custom)) {
            ...
            //$this->db->select('MATCH ('.$this->_dbprefix.$this->table_trans.'.title) AGAINST ("'.str_replace(' ',' +',$search_custom).'" IN BOOLEAN MODE) AS score_search');
            //$this->db->where('MATCH ('.$this->_dbprefix.$this->table_trans.'.title) AGAINST ("'.str_replace(' ',' +',$search_custom).'" IN BOOLEAN MODE)', NULL, FALSE);

            if(!empty($arrKey)) foreach ($arrKey as $keyword)
                $this->db->like("$this->table_trans.title", $keyword);
            /*$this->db->group_start();
            $this->db->like("$this->table_trans.meta_keyword", $search_custom);
            $this->db->or_like("$this->table_trans.title", $search_custom);
