...
class Product extends Admin_Controller
{
	...
	public function ajax_list()
	{
		...
		if (!empty($listData)) foreach ($listData as $item) {
			...
			$row['checkID'] = $item->id;
			$row['id'] = $item->id;
			$row['title'] = "<a target='_blank' href='" . getUrlProduct($item) . "' title='$item->title'>$item->title</a>";
	...
	public function ajax_edit()
	{
		...
		if (!empty($id)) {
			...
			if (!empty($oneItem->data_tags)) {
				$idTag = json_decode($oneItem->data_tags);
				$output['data_tags'] = $this->_data_category->getSelect2($idTag);
			}
