...
class Test extends Public_Controller
{
    ...
    private function getImageThumb($image = '',$width = '',$height= '', $crop = false, $watermark = false){
        ...
        if($width != 0 && $height != 0){
            ...
            return MEDIA_URL.DIRECTORY_SEPARATOR.$newImage;
        }
        else {
            $newPathImage = str_replace('//','/',MEDIA_PATH_THUMB.$image);
            if(!is_dir(dirname($newPathImage))){
                mkdir(dirname($newPathImage), 0755, TRUE);
    ...
    private function getWatermark($width = '',$height= ''){
        $CI =& get_instance();
